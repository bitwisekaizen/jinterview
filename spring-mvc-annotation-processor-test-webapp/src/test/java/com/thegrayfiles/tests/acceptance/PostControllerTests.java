...
public class PostControllerTests {
    ...
    @Test
    public void canPostFileWithReturnValue() throws IOException {
        String filename = "test-image.jpg";
        ClassPathResource file = new ClassPathResource(filename);
        TestEntity testEntityToPost = new TestEntity();
        DynamicallyGeneratedClientMethodInvoker invoker = aMethodNamed("canPostMultipartFile").withArgument(File.class, file.getFile()).withArgument(TestEntity.class, testEntityToPost);
        TestEntity postedTestEntity = invoker.invoke(TestEntity.class);
        assertEquals(postedTestEntity.getFilename(), filename);
    }
